...
export const EnrollmentService = {
    enrollInCourse: async (studentId, courseId) => {
        const isEnrolled = await EnrollmentRepository.findEnrollment(studentId, courseId);
        if (isEnrolled) {
    ...
    updateLessonProgress: async (studentId, courseId, lessonId, isCompleted) => {
        ...
        const stats = await EnrollmentRepository.calculateProgress(studentId, courseId);
        const status = stats.percentage === 100 ? "COMPLETED" : "ACTIVE";
        return await EnrollmentRepository.updateEnrollmentStats(studentId, courseId, stats.percentage, status);
    },
};
